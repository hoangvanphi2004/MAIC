...
class BatchedGPModel(gpytorch.models.ExactGP):
    ...
    def forward(self, x):
        ...
        return:
            MultivariateNormal với:
            mean  : (output_dim, N)
            covar : (output_dim, N, N)
        """
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
...
class GaussianProcess:
	def __init__(
		...
		"""
		Batched Independent Gaussian Process

		output_dim : số chiều đầu ra (vd: state_dim hoặc delta_state_dim)
		min_noise  : noise floor để đảm bảo ổn định số
		"""
		...
		# GP components (sẽ được khởi tạo ở fit lần đầu)
		self.model = None
		self.likelihood = None
		self.optimizer = None

		# Flag để biết đã init GP hay chưa (cho warm-start)
		self.is_initialized = False

	def fit(self, x_train, y_train, training_iter=10, lr=0.05):
		"""
		x_train : (N, input_dim)
		y_train : (N, output_dim)

		training_iter : số bước update ngắn (nên nhỏ, train thường xuyên)
		"""
		...
		# GPytorch batched GP yêu cầu (output_dim, N)
		y_train = y_train.transpose(0, 1)

		# ========= LẦN ĐẦU: KHỞI TẠO MODEL =========
		if not self.is_initialized:
			self.likelihood = gpytorch.likelihoods.GaussianLikelihood(
				batch_shape=torch.Size([self.output_dim])
			).to(self.device)

			# noise floor để tránh numerical instability
			self.likelihood.noise_covar.register_constraint(
				"raw_noise",
				gpytorch.constraints.GreaterThan(self.min_noise)
			)
		...
		# ========= CÁC LẦN SAU: WARM-START =========
		else:
			# cập nhật data nhưng giữ nguyên kernel / noise / lengthscale
		...
		for _ in range(training_iter):
			...
			# batched GP → mll trả về (output_dim,)
