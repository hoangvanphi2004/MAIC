...
from COMA import EpisodeMemory, ReplayBuffer, COMA
    ...
    for ep in range(episodes):
        ...
        if len(replay_buffer) > batch_size and (ep + 1) % steps_per_update == 0:
            ...
            for _ in range(updates_num):
                value_stats = agent.update_value_net(replay_buffer, batch_size)
                if value_stats:
                    cycle_value_loss.append(value_stats.get('value_loss', 0.0))
            if cycle_value_loss:
                metrics['value_losses'].append(np.mean(cycle_value_loss))

        # Update policies with COMA
        try:
            policy_stats = agent.update_policies(episode_memory)
            if policy_stats:
                metrics['policy_losses'].append(policy_stats.get('policy_loss', 0.0))
        ...
        if record_this_ep and frames:
            if imageio is None:
                ...
                # Update metric plots after saving video
                try:
                    if plt is not None:
                        fig, axes = plt.subplots(2, 2, figsize=(12, 8))
                        (ax_reward, ax_policy), (ax_value, ax_adv) = axes
                        ...
                        if policy_losses:
                            ...
                            ax_policy.set_title('COMA Policy Loss')
                        ...
                        if value_losses:
                            ax_value.plot(value_losses, marker='o', ms=3)
                            ax_value.set_title('Value Network Loss')
                            ax_value.grid(True, alpha=0.3)
                        else:
                            ax_value.set_axis_off()

                        advantages = metrics.get('advantages', [])
                        if advantages:
                            ax_adv.plot(advantages, marker='o', ms=3)
                            ax_adv.set_title('Counterfactual Advantage')
