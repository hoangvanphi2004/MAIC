...
class AttentionSAC(object):
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        # print("acs:", [a.shape for a in acs], "next_acs:", [na.shape for na in next_acs])
        # print("Obs:", [o.shape for o in obs], "Next obs:", [no.shape for no in next_obs])
    ...
    def init_from_env(cls, env, gamma=0.95, tau=0.01,
        ...
        if env.observation_space.__class__.__name__ == 'Dict':
            for id, obsp in env.observation_space.items():
                acsp = env.action_space[id]
                agent_init_params.append({'num_in_pol': math.prod(obsp['image'].shape),
                                        'num_out_pol': acsp.n})
                sa_size.append((math.prod(obsp['image'].shape), acsp.n))
        else:
            for acsp, obsp in zip(env.action_space,
                                env.observation_space):
                if env.observation_space[0].__class__.__name__ == 'Dict':
                    agent_init_params.append({'num_in_pol': math.prod(obsp['pov'].shape),
                                            'num_out_pol': acsp.n})
                    sa_size.append((math.prod(obsp['pov'].shape), acsp.n))
                else:
