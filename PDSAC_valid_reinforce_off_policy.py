...
class SAC_REINFORCE:
    ...
    """def compute_baseline(self, states, agent_idx):
        
        Tính baseline cho agent_idx:
        baseline = E[Q(s, u)] theo policy hiện tại của tất cả agents
        
        B = states.size(0)  # batch size (số timesteps)
        
        with torch.no_grad():
            # Sample multiple actions theo policy hiện tại để estimate expected Q-value
            baseline_values = torch.zeros(B, device=self.device)
            n_samples = 10  # Số lượng samples để estimate expectation
            
            for _ in range(n_samples):
                # Sample actions cho tất cả agents theo policy hiện tại
                sampled_actions = []
                for i, actor in enumerate(self.actors):
                    agent_state = states[:, i]  # [B, C, H, W]
                    action_probs = actor(agent_state)  # [B, n_actions]
                    dist = Categorical(action_probs)
                    sampled_action = dist.sample()  # [B]
                    sampled_actions.append(sampled_action)
                
                sampled_actions = torch.stack(sampled_actions, dim=1)  # [B, num_agents]
                
                # Tính Q-value cho sampled actions
                sampled_actions_oh = F.one_hot(sampled_actions.long(), num_classes=self.n_actions).float()
                q1, q2 = self.critic(states, sampled_actions_oh)
                q_value = torch.min(q1, q2).squeeze(-1)  # [B]
                
                baseline_values += q_value
            
            # Average qua các samples
            baseline_values = baseline_values / n_samples
    ...
    def update_reinforce(self, episode_memory):
        ...
        states_np = np.array(episode_memory.states)  # [T, num_agents, H, W, C]
        actions_np = np.array(episode_memory.actions)  # [T, num_agents]
        rewards_seq = np.array(episode_memory.rewards)  # [T, num_agents]
        
        # Convert to tensors
        states = torch.FloatTensor(states_np).permute(0, 1, 4, 2, 3).to(self.device)  # [T, num_agents, C, H, W]
        actions = torch.LongTensor(actions_np).to(self.device)  # [T, num_agents]
        
        # Tính Q-values cho các actions thực tế
        with torch.no_grad():
            actions_oh = F.one_hot(actions.long(), num_classes=self.n_actions).float()
            q1, q2 = self.critic(states, actions_oh)
            q_values = torch.min(q1, q2).squeeze(-1)  # [T]
        ...
        for a_i, actor in enumerate(self.actors):
            states_a = states[:, a_i]  # [T, C, H, W]
            actions_a = actions[:, a_i]  # [T]
