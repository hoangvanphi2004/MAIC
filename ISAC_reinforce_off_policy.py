...
class Critic(nn.Module):
    """Independent critic for a single agent."""
    def __init__(self, state_shape, action_dim, hidden_dim=256):
        ...
        self.q1_fc1 = nn.Linear(input_size, hidden_dim)
        self.q1_fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.q1_out = nn.Linear(hidden_dim, 1)

        self.q2_fc1 = nn.Linear(input_size, hidden_dim)
    ...
    def forward(self, state, action):
        ...
        Args:
            state: (B, C, H, W) - single agent's observation
            action: (B, action_dim) - single agent's action (one-hot)
        """
        state_emb = self.encode_state(state)
        
        if action.dim() == 1:
            action = F.one_hot(action.long(), num_classes=action.size(-1)).float()
        elif action.dim() == 2 and action.dtype == torch.long:
            action = F.one_hot(action.long(), num_classes=action.max().item() + 1).float()
...
class ISAC_REINFORCE:
    def __init__(self, state_shape, action_dim, num_agents=2, hidden_dim=128, lr=3e-4, gamma=0.99,
        ...
        # Independent critics - one per agent
        self.critics = [Critic(state_shape, action_dim, hidden_dim=256).to(self.device)
                        for _ in range(self.num_agents)]
        self.critic_targets = [Critic(state_shape, action_dim, hidden_dim=256).to(self.device)
                               for _ in range(self.num_agents)]
    ...
    def update_reinforce(self, episode_memory):
        ...
        for a_i, actor in enumerate(self.actors):
            ...
            # One-hot actions
            actions_oh = F.one_hot(actions_a.long(), num_classes=self.n_actions).float()

            # Advantage without baseline: use Q(s,a) directly
            with torch.no_grad():
                q1_sa, q2_sa = self.critics[a_i](states_a, actions_oh)
                advantages = torch.min(q1_sa, q2_sa).squeeze(-1)
            ...
            policy_loss_a = -(new_log_probs * advantages.detach()).mean() - alpha * entropy
    ...
    def update_sac(self, replay_buffer, batch_size=64):
        """Update independent SAC critics - one per agent."""
        ...
        for agent_id in range(self.num_agents):
            ...
            state_i = state[:, agent_id]  # (B, C, H, W)
            action_i = action[:, agent_id]  # (B,)
            reward_i = reward[:, agent_id] if reward.dim() == 2 else reward  # (B,)
            next_state_i = next_state[:, agent_id]  # (B, C, H, W)
            done_i = done[:, agent_id] if done.dim() == 2 else done  # (B,)
            
            # Compute target Q-value
            with torch.no_grad():
                next_action_i, next_log_prob_i, _ = self.actors[agent_id].sample(next_state_i)
                next_action_oh_i = F.one_hot(next_action_i.long(), num_classes=self.n_actions).float()
                
                next_q1_target, next_q2_target = self.critic_targets[agent_id](next_state_i, next_action_oh_i)
                next_q_target = torch.min(next_q1_target, next_q2_target)
                
                alpha = self.alpha.detach() if isinstance(self.alpha, torch.Tensor) else self.alpha
                target_q = next_q_target - alpha * next_log_prob_i.unsqueeze(1)
                target_q_value = reward_i.unsqueeze(1) + (1 - done_i.unsqueeze(1)) * self.gamma * target_q
            ...
            for target_param, param in zip(self.critic_targets[agent_id].parameters(), 
    ...
    def load(self, filename):
        ...
        critic_states = checkpoint.get('critics', None)
        if critic_states is not None:
            for c, st in zip(self.critics, critic_states):
                c.load_state_dict(st)
        
        critic_target_states = checkpoint.get('critic_targets', None)
        if critic_target_states is not None:
            for ct, st in zip(self.critic_targets, critic_target_states):
