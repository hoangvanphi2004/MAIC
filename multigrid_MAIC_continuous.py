...
import numpy as np
import torch
from MAIC_continuous import MAIC, EpisodeMemory, ReplayBuffer
try:
...
def run_training(
	...
	for ep in range(episodes):
		...
		avg_alpha1 = np.mean(cycle_alpha1) if cycle_alpha1 else 0.0
		avg_alpha2 = np.mean(cycle_alpha2) if cycle_alpha2 else 0.0
		print(f'[Ep {ep+1:4d}] Critic Loss: {avg_critic_loss:.4f} | Q-value: {avg_q:.4f} | Alpha1: {avg_alpha1:.6f} | Alpha2: {avg_alpha2:.6f}')
			...
			if imageio is None:
				...
				try:
					if plt is not None:
						...
						if alpha_vals:
							ax_alpha.plot(alpha_vals, marker='o', ms=3, color='purple')
							ax_alpha.set_title('Alpha (combined)')
						...
						if alpha1_vals:
							ax_alpha1.plot(alpha1_vals, marker='o', ms=3, color='red')
							ax_alpha1.set_title('Alpha1 (entropy)')
							ax_alpha1.grid(True, alpha=0.3)
						else:
							ax_alpha1.set_axis_off()
						
						alpha2_vals = metrics.get('alpha2_values', [])
						if alpha2_vals:
							ax_alpha2.plot(alpha2_vals, marker='o', ms=3, color='green')
							ax_alpha2.set_title('Alpha2 (info bonus)')
							ax_alpha2.grid(True, alpha=0.3)
						else:
							ax_alpha2.set_axis_off()
